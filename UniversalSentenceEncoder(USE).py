...
Original file is located at
    https://colab.research.google.com/drive/10HHHG_5s2Ix45opiQFLdDH2FzJDQt7uz
"""
...
def closest_question(input_question, question_list):
    ...
    vec1 = get_features(input_question)[0]
    for i in range(0, len(question_list)):
        vec2 = get_features(question_list[i])[0]
        if cosine_similarity(vec1, vec2) > max:
            max = cosine_similarity(vec1, vec2)
...
def closest_question_using_base_vectors(input_question, question_list):
    max = -1
    most_probable_question = ""
    vec1 = get_features(input_question)[0]
    for i in range(0, len(question_list)):
...
test_similiarity('where to mail my i-20/visa documents?', 'where to mail my i-20/visa documents?')
...
question_list, answer_list = populate_FAQ_data('GSU_FAQ_Data.csv')
BASE_VECTORS = get_features(question_list)
temp = closest_question_using_base_vectors('where to mail my i-20/visa documents?', question_list)
print(temp)

temp = closest_question_using_base_vectors('When can I apply for Housing?', question_list)
print(temp) 